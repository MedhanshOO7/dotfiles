...
VIDEO_OPTS="no-audio loop hwdec=auto-safe really-quiet panscan=1.0 scale=bilinear cscale=bilinear dscale=bilinear correct-downscaling=no sigmoid-upscaling=no load-scripts=no no-sub no-osd-bar osd-level=0 stop-screensaver=no framedrop=vo opengl-pbo=yes interpolation=no video-sync=audio"
...
nohup mpvpaper -p -a MAX -o "$VIDEO_OPTS" ALL "$video_path" >/dev/null 2>&1 &
EOF
