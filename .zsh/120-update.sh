...
UPDATE_CONFIRM="${UPDATE_CONFIRM:-0}"
...
update_post_hook() {
    ...
    if [[ -d "$repo_dir/.git" ]]; then
        ...
        git clone https://github.com/0x7375646F/Linuwu-Sense.git "$repo_dir" || return 2
    fi
    ...
    if [[ "$_U_MODE" == "gui" ]]; then
        ...
        if ! echo "$pass" | sudo -S true 2>/dev/null; then
            zenity --error \
                ...
                --text="Incorrect password. Update aborted." \
...
_u_core() {
    ...
    _u_step 1 "Keyring" "Updated" \
        "pacman -S archlinux-keyring failed" -- \
        sudo pacman "${pacman_flags[@]}" archlinux-keyring || return 1
    ...
    _u_step 1 "System upgrade" "pacman -Syu complete" \
        "pacman -Syu failed" -- \
        sudo pacman "${syu_flags[@]}" || return 1
    ...
    if command -v "$UPDATE_AUR_TOOL" &>/dev/null; then
        ...
        _u_step 0 "Orphans" "Removed unused dependencies" \
            "Orphan removal failed" -- \
            "$UPDATE_AUR_TOOL" -Yc --noconfirm

        _u_step 0 "Cache" "Package cache cleaned" \
            "Cache clean failed" -- \
...
Commands:
  cli   Run update in terminal
  gui   Run update with Zenity GUI
  dry   Dry-run mode
